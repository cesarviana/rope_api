...
describe('test get data', () => {
    ...
    it('/execution interactions', async () => {
        ...
        const executionsResponse = await api.get(`/interaction/byExecution/${(interaction.executionId)}`);
        expect(executionsResponse.body.length).to.be.greaterThan(0);
    });

    it('/execution by user', async () => {
        const user = await getOne('user');
        const executionsResponse = await api.get(`/taskExecution/byUser/${user.id}`);
        expect(executionsResponse.body.length).to.be.greaterThan(0);
    });
